...
void TutorialApp::RenderImGUI()
{
	...
	ImGui::Begin("D");                          // Create a window called "Hello, world!" and append into it.

	ImGui::Text("Camera");

	ImGui::InputFloat3("Degree",&m_RotationDegree.x);
	if (ImGui::Button("SetRotation"))                            // Buttons return true when clicked (most widgets return true when edited/activated)
	{
		m_Camera.m_Rotation.x = XMConvertToRadians(m_RotationDegree.x);
		m_Camera.m_Rotation.y = XMConvertToRadians(m_RotationDegree.y);
		m_Camera.m_Rotation.z = XMConvertToRadians(m_RotationDegree.z);
	}
...
bool TutorialApp::InitScene()
{
	...
	HR_T( CompileShaderFromFile( L"../shaders/10_BasicVertexShader.hlsl", "main", "vs_4_0", &vertexShaderBuffer ));
	...
	HR_T( CompileShaderFromFile( L"../shaders/10_BasicPixelShader.hlsl", "main", "ps_4_0", &pixelShaderBuffer ));
	HR_T(m_pDevice->CreatePixelShader(pixelShaderBuffer->GetBufferPointer(),
		pixelShaderBuffer->GetBufferSize(), NULL, &m_pPixelShader));
	SAFE_RELEASE(pixelShaderBuffer);

	HR_T( CompileShaderFromFile(L"../shaders/10_SolidPixelShader.hlsl", "main", "ps_4_0", &pixelShaderBuffer));	
	HR_T(m_pDevice->CreatePixelShader(pixelShaderBuffer->GetBufferPointer(),
