...
void TutorialApp::OnRender()
{
	...
	m_pDeviceContext->IASetVertexBuffers(0, 1, &m_pQuadVertexBuffer, &m_QuadVertextBufferStride, &m_QuadVertextBufferOffset);
...
{	
	...
	//   |/    |
	//	 2-----3
	CubeVertex QuadVertices[] =
	{											
		CubeVertex(Vector3(-1.0f,  1.0f, 1.0f), Vector4(0.0f, 0.0f, 1.0f, 1.0f)),	// Left Top 
		CubeVertex(Vector3( 1.0f,  1.0f, 1.0f), Vector4(0.0f, 1.0f, 0.0f, 1.0f)),	// Right Top
		CubeVertex(Vector3(-1.0f, -1.0f, 1.0f), Vector4(1.0f, 0.0f, 0.0f, 1.0f)),	// Left Bottom
		CubeVertex(Vector3( 1.0f, -1.0f, 1.0f), Vector4(0.0f, 0.0f, 1.0f, 1.0f))	// Right Bottom
	};

	D3D11_BUFFER_DESC vbDesc = {};
	vbDesc.ByteWidth = sizeof(CubeVertex) * ARRAYSIZE(QuadVertices);
	vbDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vbDesc.Usage = D3D11_USAGE_DEFAULT;	
	D3D11_SUBRESOURCE_DATA vbData = {};
	vbData.pSysMem = QuadVertices;	// 배열 데이터 할당.
	HR_T(m_pDevice->CreateBuffer(&vbDesc, &vbData, &m_pQuadVertexBuffer));  
	m_QuadVertextBufferStride = sizeof(CubeVertex);		// 버텍스 버퍼 정보
