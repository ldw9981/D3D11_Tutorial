...
bool TutorialApp::InitScene()
{
	...
	// 1. Render() 에서 파이프라인에 바인딩할 버텍스 버퍼및 버퍼 정보 준비		
	CubeVertex vertices[] = // Local or Object or Model Space    position
	{
	...
	m_VertexBufferStride = sizeof(CubeVertex); 	// 버텍스 버퍼의 정보
	...
	HR_T( CompileShaderFromFile(L"../shaders/05_BasicVertexShader.hlsl", "main", "vs_4_0", &vertexShaderBuffer));
	...
	HR_T( CompileShaderFromFile(L"../shaders/05_BasicPixelShader.hlsl", "main", "ps_4_0", &pixelShaderBuffer));
	...
	m_Projection = XMMatrixPerspectiveFovLH(XM_PIDIV2, m_ClientWidth / (FLOAT)m_ClientHeight, 1.0f, 10000.0f);
	return true;
}
